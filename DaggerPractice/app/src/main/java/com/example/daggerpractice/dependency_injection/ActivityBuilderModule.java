...
{
    // As AuthViewModelModule is only going to scoped to only to AuthComponent and to make sure that we have to explicitly mention the module name
    // in the @ContributesAndroidInjector annotation. Now only the AuthActivity sub component will be able to use this ViewModel. It is where we
    // define the SubComponent. In general we make sub component for organisation and for scoping.
